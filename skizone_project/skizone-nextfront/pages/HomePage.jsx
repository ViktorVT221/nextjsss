...
function HomePage() {
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      await axios.post('http://localhost:5001/api/cart/add', { productId, quantity: 1 }, {
  ...
  return (
    <div className={styles.homePage}>
      ...
      <div className={styles.banner}>
        ...
        <p className={styles.siteSubtitle}>Тут можна знайти все необхідне для катання на лижах: одяг, аксесуари та спорядження</p>
      </div>
        ...
        {filteredProducts.length === 0 ? (
          ...
          filteredProducts.map(product => (
            <div key={product._id} className={styles.productCard}>
              {product.imagePath ? (
                <img src={`http://localhost:5001/${product.imagePath}`} alt={product.name} className={styles.productImage} />
              ) : (
              ...
              <button className={styles.addCartBtn} onClick={() => handleAddToCart(product._id)}>В кошик</button>
            </div>
          ))
        )}
      </div>
